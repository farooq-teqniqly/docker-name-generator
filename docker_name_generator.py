...
def __read_file(path, target_list):
    """
    Reads a file and appends each line to a list.

    Args:
        path: The path to the file to read.
        target_list: The list to append each line to.
    ...
    with io.open(path, "r", encoding="utf8") as f:
        try:
            for line in f:
                target_list.append(line.replace(" ", "").strip())
...
def load_files(names_file, nouns_file):
    ...
    return list(set(islice(names, max_results_to_return))), list(set(islice(nouns, max_results_to_return)))
